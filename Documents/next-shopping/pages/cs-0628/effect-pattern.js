...
export default function EffectPattern() {
  ...
  // useEffect(() => {
  //   console.log('每次渲染(render)之後(after)，會再執行第一傳入參數函式其中程式碼')
  // })
  ...
  // useEffect(() => {
  //   console.log('首次渲染(render)之後(after)，執行一次第一傳入參數函式其中程式碼，之後不會再執行')
  // }, [])
  ...
  useEffect(() => {
    console.log('首次渲染(render)之後(after)，執行一次第一傳入參數函式其中程式碼。之後當相依變數有更動之後(after)，會再執行一次。')
  }, [total])
