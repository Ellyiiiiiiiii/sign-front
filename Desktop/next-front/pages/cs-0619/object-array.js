  ...
  return (
    <>
      ...
      <button onClick={() => {
        ...
        // 3.時間日期轉毫秒整數值(或組合為時間字串或數字字串) new Data() 或 Date.now()
      ...
      <br />
      <button onClick={() => {
        // 1. 2.過濾後剩下除了文字為b之外的物件資料 === 刪除文字為b的物件資料
        const nextData = data.filter((v, i) => {
          return v.text !== 'b'
        })
        // 3.
      ...
      <br />
      <button onClick={() => {
        // 第1種: 使用slice(分割). 公式: array.slice(startIndex, endIndex)，注意不包含endIndex成員
        // 1. 尋找是否有id = 2
